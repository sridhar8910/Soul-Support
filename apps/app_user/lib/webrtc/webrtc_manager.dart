...
class WebRTCManager {
  ...
  /// Check if WebRTC is supported on current platform
  static bool isPlatformSupported() {
    // Web (Chrome, Safari, Firefox) - supported
    if (kIsWeb) {
      return true;
    }
    ...
    // Desktop platforms (Windows, macOS, Linux) - not supported by flutter_webrtc
    return false;
  }
  ...
  Future<bool> initialize({
    ...
    // Check platform support before attempting initialization
    if (!isPlatformSupported()) {
      final platform = kIsWeb 
          ? 'web' 
          : defaultTargetPlatform.toString().replaceAll('TargetPlatform.', '');
      final errorMsg = 'WebRTC is not supported on $platform. '
          'Please use Chrome browser or mobile app for calls.';
      print('[WebRTC Manager] Platform not supported: $platform');
  ...
  Future<void> createOffer() async {
    try {
      // Message replay in backend handles missed offers, so no delay needed
